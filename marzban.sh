...
services:
  marzban:
    image: arsyakmar/marzban:${marzban_version}
    restart: always
    env_file: .env
    network_mode: host
    volumes:
...
EOF
        ...
        if [ "$marzban_version" == "latest" ]; then
            yq -i '.services.marzban.image = "arsyakmar/marzban:latest"' "$docker_file_path"
        else
            yq -i ".services.marzban.image = \"arsyakmar/marzban:${marzban_version}\"" "$docker_file_path"
        fi
