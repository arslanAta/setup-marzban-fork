...
install_marzban_script() {
    FETCH_REPO="arslanAta/setup-marzban-fork"
...
install_marzban() {
    ...
    # Fetch releases
    FILES_URL_PREFIX="https://raw.githubusercontent.com/arslanAta/marzban-fork/main"
...
services:
  marzban:
    image: arslanata/marzban:${marzban_version}
    restart: always
    env_file: .env
    network_mode: host
    volumes:
...
EOF
        ...
        if [ "$marzban_version" == "latest" ]; then
            yq -i '.services.marzban.image = "arslanata/marzban:latest"' "$docker_file_path"
        else
            yq -i ".services.marzban.image = \"arslanata/marzban:${marzban_version}\"" "$docker_file_path"
        fi
...
install_command() {
    ...
    check_version_exists() {
        local version=$1
        repo_url="https://api.github.com/repos/arslanAta/marzban-fork/releases"
